...
<div class="js-engine-message callout {mapMessageTypeToClass(messageWrapper.message.type)}"
	 data-callout="{messageWrapper.message.type}">
	<div class="js-engine-message-header callout-title">
		<div class="js-engine-message-icon callout-icon"
			 use:useIcon={mapMessageTypeToIcon(messageWrapper.message.type)}></div>
		<span class="js-engine-message-title">{messageWrapper.message.title}</span>
		{#if showDeleteButton}
			<div class="js-engine-message-delete-icon callout-icon"
				 ...
				 use:useIcon={"x"} aria-label="remove" role="button" tabindex="0"></div>
		{/if}
	</div>
	...
	<div class="js-engine-message-content callout-content">
		...
		<div class="js-engine-message-detail">
			<div class="js-engine-message-detail-row" on:click={() => showMore = !showMore}
				 aria-expanded={showMore ? "true" : "false"}
				 on:keydown={(e) => e.key === ' ' ? showMore = !showMore : undefined} role="button" tabindex="0">
				<span>Details</span>
